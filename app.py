...
# ========================================
# DIGITAL PRODUCTS ACCESS ROUTES
# ========================================

@app.route('/my_digital_products')
@login_required
def my_digital_products():
    """Display user's purchased digital products"""
    # Get completed orders for digital products
    digital_orders = ProductOrder.query.join(Product).filter(
...
@app.route('/download_digital_product/<int:order_id>')
@login_required
def download_digital_product(order_id):
    """Allow user to download their digital product"""
    ...
    # For now, we'll create a download link or redirect to the product content
    # You can modify this based on how you want to handle digital product delivery
    
    # Option 1: Redirect to a content page
    return redirect(url_for('view_digital_product', order_id=order_id))
    
    # Option 2: Generate download link (uncomment if you have files to serve)
    # return serve_digital_file(order.product.id)

@app.route('/view_digital_product/<int:order_id>')
@login_required
def view_digital_product(order_id):
    """View digital product content"""
    # Verify order belongs to user and is completed
    order = ProductOrder.query.filter_by(
        id=order_id,
        user_id=current_user.id,
    ...
    return render_template('view_digital_product.html', order=order, product=order.product)

# Optional: Serve digital files if you have downloadable content
@app.route('/serve_digital_file/<int:product_id>')
@login_required
def serve_digital_file(product_id):
    """Serve digital file for download"""
    # Verify user has purchased this product
    order = ProductOrder.query.join(Product).filter(
        ProductOrder.user_id == current_user.id,
        ProductOrder.status == 'completed',
    ...
    if not order:
        flash('You do not have access to this digital product.', 'error')
        return redirect(url_for('my_digital_products'))
    
    # Here you would serve the actual file
    # For example, if you store files in a 'digital_products' folder:
    try:
        digital_folder = os.path.join(app.config['UPLOAD_FOLDER'], 'digital_products')
        filename = f"product_{product_id}.zip"  # or whatever file format you use
        
        if os.path.exists(os.path.join(digital_folder, filename)):
            return send_from_directory(
                digital_folder, 
                filename, 
                as_attachment=True,
                download_name=f"{order.product.name}.zip"
            )
        else:
            flash('Digital product file not found. Please contact support.', 'error')
            return redirect(url_for('my_digital_products'))
    except Exception as e:
        flash('Error accessing digital product. Please contact support.', 'error')
        return redirect(url_for('my_digital_products'))
